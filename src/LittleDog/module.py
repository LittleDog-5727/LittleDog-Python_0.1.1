...
def show_dependencies():
    print("opencv-contrib-python numpy matplotlib google-generativeai")
    print("python>=3.9")
...
def initialize_log():
    ...
    cache_dir = os.path.join(working_directory, 'LittleDogAI-logs')
    # Create the directory if it doesn't exist
    if not os.path.exists(cache_dir):
        os.makedirs(cache_dir)
    # Define the log file path
    log_file_path = os.path.join(cache_dir, 'log.txt')
    # Create the log file if it doesn't exist
    if not os.path.exists(log_file_path):
        with open(log_file_path, 'w') as file:
            file.write("LittleDogAI-Log")  # Create an empty log file
    return log_file_path

def write_to_log(message):
    log_file_path = initialize_log()
...
# Eazy code withLittleDogAI: (Integrated with gemini)
def littledogai_lilfy(code:str = ".", warings:bool = True, dependency_list:bool = True):
        # Checking the code variable
        if code == ".":
            print("Please provide the instructions for the program")
            write_to_log("No code instruction given. Program ended")
            return
        ...
        # Create prompt parts
        # Checking if the user wants to know the dependencies
        # If no:
        if dependency_list == False:
            write_to_log('dependency instructions FALSE')
            prompt_text = (
                "The text in quotes (eg: 'a number x + a number z = number c. Display z') is the instructions of how the code must work."
                "Using those instructions, give the PYTHON code on how that has to work."
                "Give output in just plain text (please DONT PUT THE CODE IN '```', just give it as it is). Don't give any additional information. However, you can add comments in the code"
                "Remainder: please DONT PUT THE CODE IN '```' or dont give the code output in the usual code box. Also Make sure that the code runs directly and dont use if __name__ == '__main__' "
                "Code Instructions : '{}'\n".format(code)
            )

        # if yes:
        else:
              write_to_log('dependency instructions TRUE')
              prompt_text = (
                "The text in quotes (eg: 'a number x + a number z = number c. Display z') is the instructions of how the code must work."
                "Using those instructions, give the PYTHON code on how that has to work."
                "Give output in just plain text (please DONT PUT THE CODE IN '```', just give it as it is). Don't give any additional information. However, you can add comments in the code"
                "Also, make a variable in string with the instructions to run the code like installing some dependencies(in this case, include pip statement like pip install opencv, etc), where to run the code,etc AT THE START OF THE CODE EVEN BEFORE IMPORTING THE LIBRARIES"
                "Print that variable at first. This also must happen at the start of the code.  Also Make sure that the code runs directly and dont use if __name__ == '__main__' "
                "Remainder: please DONT PUT THE CODE IN '```' or dont give the code output in the usual code box and store the instructions in a variable which must be printed"
                "Code Instructions : '{}'\n".format(code)
            )
        ...
        lilfied = response.text
        write_to_log("CODE:")
        write_to_log(lilfied)

        # Checking if the user wants to see warning messages
        if warings == True:
            write_to_log("warnings TRUE")
            print("WARNING: The code generated might not be the same everytime so it is advised to save the code displayed safely. But, it is still stord in the log file.")
            print("Code that is being executed:")
            print(lilfied)

        write_to_log("warnings FALSE")
        # Executing the output code
        exec(lilfied)

def littledogai_lilfy_IMAGE(code:str = ".", image_path:str = ".", warings:bool = True, dependency_list:bool = True):
        # Checking the code variable
        if code == ".":
            print("Please provide the instructions for the program")
            write_to_log("No code instruction given. Program ended")
